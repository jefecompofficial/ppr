...
\begin{frame}{Exemplos de Requisitos Funcionais e Não-Funcionais}

\only<1>{
\begin{alertblock}{Exemplo de Requisito Funcional}
{
\footnotesize

\textbf{RF01} - Tela de Cadastro de Produto;\\
\textbf{Descrição}: A tela de cadastro de produto fornece uma interface gráfica que permite que um usuário do sistema cadastre um novo produto na base de dados. Para tal, a tela de cadastro de produto possui os seguintes elementos:\\

\textbf{Código do produto}: O código do produto deve possuir denominação de texto simples ``Código do Produto", além de uma caixa de texto para informar o código do produto a ser cadastrado. A caixa de texto deve permitir a visualização de 10 caracteres. A caixa de texto deve permitir somente a entrada de códigos com o seguinte formato: \textbf{PRD-XXXXXXX}. O termo \textbf{XXXXX} tem um tamanho fixo de sete (7) caracteres, e pode ser composto por: (a) somente letras maiúsculas de A a Z; (b) somente números de zero (0) a nove (9); ou (c) uma combinação de letras maiúsculas de A a Z e números de zero (0) a nove (9). Em qualquer um dos casos (de \textbf{(a)} a \textbf{(c)}) letras e números podem se repetir. Exemplos de códigos válidos: \textbf{PRD-ABCDEFG}, \textbf{PRD-0001234}, \textbf{PRD-AZ4576D}.\\
...
\begin{alertblock}{Exemplo de Requisito Não-Funcional}
{
\footnotesize

\textbf{RNF01} - O sistema deve produzir o relatório de lista de produtos em, no máximo, 3 segundos;\\
\textbf{Descrição}: Após o usuário entrar na opção de menu (Relatórios -> Lista de Produtos) o sistema deverá realizar o processo de produção do relatório citado em, no máximo, 3 segundos. Em caso de falha na produção do relatório de Lista de Produtos, o sistema deve notificar o uuário com uma mensagem de erro, indicando o motivo pelo qual o relatório não pode ser produzido. A mensagem não deve conter elementos técnicos, e utilizar uma linguagem que esteja no domínio do usuário do sistema. Além disso, a notificação de erro ao usuário deve também ocorrer em, no máximo, 3 segundos.
}
